...
/** Thông báo chuẩn khi backend bật auth mà FE chưa có/ sai Admin API token. */
export const ADMIN_TOKEN_REQUIRED_MSG =
 'Chưa cấu hình Admin API token. Hãy lưu token (nút khóa trên thanh công cụ) trước khi sử dụng chức năng này.';
...
async function req<T>(method: string, path: string, body?: unknown): Promise<T> {
 ...
 // 401 = backend bật auth nhưng FE thiếu/sai token → thông báo rõ ràng thay vì lỗi fail.
 if (res.status === 401) {
 throw new Error(ADMIN_TOKEN_REQUIRED_MSG);
 }
...
export interface Issue {
 ...
 elements?: { selector: string; outerHTML: string; text?: string; boundingRect?: Record<string, number> }[];
